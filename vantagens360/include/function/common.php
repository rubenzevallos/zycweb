...
function share_mail($team) {
	...
	if ( $team['id'] ) {
		$pre[] = "Os clientes hoje estão：{$team['title']}";
		$pre[] = "Acho que você vai estar interessado em：";
		$pre[] = $INI['system']['wwwprefix'] . "/team.php?id={$team['id']}&r={$login_user_id}";
		$pre = mb_convert_encoding(join("\n\n", $pre), 'GBK', 'UTF-8');
		$sub = "Você está interessado em：{$team['title']}";
	} else {
		$sub = $pre[] = $team['title'];
	}
...
function state_explain($team, $error='false') {
	...
	switch($state) {
		case 'none': return 'Em Andamento';
		case 'soldout': return 'Esgotado';
		case 'failure': if($error) return 'Falha';
...
function get_zones($zone=null) {
	$zones = array(
			'city' => 'Cidade',
			'group' => 'Categoria de Clientes',
			'public' => 'Forum',
			'grade' => 'Avaliação de Usuário',
			'express' => 'Express',
			);
