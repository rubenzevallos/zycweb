...
<div id="bd" class="cf">
<?php if(is_newbie()){?><div id="sysmsg-guide"><div class="link"><a href="/help/tour.php"></a></div><a id="sysmsg-guide-close" href="javascript:void(0);" class="close">Fechar</a></div><?php }?>

<?php if($team['close_time']){?>
<div id="sysmsg-tip" class="sysmsg-tip-deal-close"><div class="sysmsg-tip-top"></div><div class="sysmsg-tip-content"><div class="deal-close"><div class="focus">Desculpe, tarde demais! <br /> esta oferta acabou!</div><div id="tip-deal-subscribe-body" class="body"><form id="tip-deal-subscribe-form" method="post" action="/subscribe.php" class="validator"><table><tr><td>N&atilde;o perca outra oferta, inscreva-se para receber nossas ofertas: :&nbsp;</td><td><input type="text" name="email" class="f-text" value="" require="true" datatype="email" /></td><td>&nbsp;<input class="commit" type="submit" value="Subscribe" /></td></tr></table></form></div></div><span id="sysmsg-tip-close" class="sysmsg-tip-close">Close</span></div><div class="sysmsg-tip-bottom"></div></div>
<?php }?>

<?php if($order){?>
<div id="sysmsg-tip" ><div class="sysmsg-tip-top"></div><div class="sysmsg-tip-content">Existe uma compra que ainda n&atilde;o foi paga.<a href="/order/index.php?id=<?php echo $order['id']; ?>">Por favor, verifique suas compras e efetue o pagamento!</a><span id="sysmsg-tip-close" class="sysmsg-tip-close">Fechar</span></div><div class="sysmsg-tip-bottom"></div></div><div id="deal-default">
<?php }?>
	...
	<div id="deal-default">
		...
		<div id="content">
			<div id="deal-intro" class="cf">
                <h1><?php if($team['close_time']==0){?><a class="deal-today-link" href="/team.php?id=<?php echo $team['id']; ?>">Oferta do Dia:</a><?php }?><?php echo $team['title']; ?></h1>
                <div class="main">
                    <div class="deal-buy">
					...
					<?php } else { ?>
                        <p class="deal-price"><strong><?php echo $currency; ?><?php echo moneyit($team['team_price']); ?></strong><span><a <?php echo $team['begin_time']<time()?'href="/team/buy.php?id='.$team['id'].'"':''; ?>><img src="/static/css/i/button-deal-buy.png" /></a></span></p>
					<?php }?>
                    </div>
                    ...
                    <div class="deal-box deal-timeleft deal-off" id="deal-timeleft" curtime="<?php echo $now; ?>000" diff="<?php echo $diff_time; ?>000">
						...
						<div class="limitdate"><p class="deal-buy-ended"><?php echo date('d-m-Y', $team['close_time']); ?><br><?php echo date('H:i:s', $team['close_time']); ?></p></div>
					</div>
					<?php } else { ?>
                    <div class="deal-box deal-timeleft deal-on" id="deal-timeleft" curtime="<?php echo $now; ?>000" diff="<?php echo $diff_time; ?>000">
						...
						<?php if($left_day>0){?>
							<li><span><?php echo $left_day; ?></span>dias</li><li><span><?php echo $left_hour; ?></span>horas</li><li><span><?php echo $left_minute; ?></span>minutos</li>
						<?php } else { ?>
							<li><span><?php echo $left_hour; ?></span>horas</li><li><span><?php echo $left_minute; ?></span>minutos</li><li><span><?php echo $left_time; ?></span>segundos</li>
						<?php }?>
						</ul></div>
					</div>
					<?php }?>
				...
				<?php if($team['close_time']==0){?>
					...
					<div class="deal-box deal-status" id="deal-status">
						<p class="deal-buy-tip-top"><strong><?php echo $team['now_number']; ?></strong> compras </p>
						...
						<p class="deal-buy-tip-btm">Esta oferta precisa de <strong><?php echo $team['min_number']-$team['now_number']; ?></strong> compras para valer!</p>
					</div>
					<?php } else { ?>
					<div class="deal-box deal-status deal-status-open" id="deal-status">
						<p class="deal-buy-tip-top"><strong><?php echo $team['now_number']; ?></strong> compras </p>
						<p class="deal-buy-on" style="line-height:200%;"><img src="/static/css/i/deal-buy-succ.gif"/> A oferta j&aacute; est&aacute; ativa! <?php if($team['max_number']>$team['now_number']||$team['max_number']==0){?><br/>Se voc&ecirc; j&aacute; comprou, favor aguardar até 24h ap&oacute;s o final do tempo restante, para poder imprimir seu cupom. <?php }?></p>
					</div>
					<?php }?>
				<?php } else { ?>
					<div class="deal-box deal-status deal-status-<?php echo $team['state']; ?>" id="deal-status"><div class="deal-buy-<?php echo $team['state']; ?>"></div><p class="deal-buy-tip-total">Total <strong><?php echo $team['now_number']; ?></strong> compras </p></div>
				<?php }?>
				</div>
                <div class="side">
