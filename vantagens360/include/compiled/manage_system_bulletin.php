            ...
            <div class="box-content">
                ...
                <div class="sect">
					...
					<input type="hidden" name="id" value="<?php echo $system['id']; ?>" />
						...
						<div class="wholetip clear"><h3>2. Cidades</h3></div>
					<?php if(is_array($hotcities)){foreach($hotcities AS $one) { ?>
                        <div class="field">
                            <label><?php echo $one['name']; ?></label>
                            <div style="float:left;"><textarea cols="45" rows="5" name="bulletin[<?php echo $one['id']; ?>]" id="system-create-location" class="xheditor-simple"><?php echo htmlspecialchars($INI['bulletin'][$one['id']]); ?></textarea></div>
						</div>
					<?php }}?>
                        <div class="act">
                            <input type="submit" value="Postar" name="commit" id="system-submit" class="formbutton"/>
                        </div>
                    </form>
                </div>
            </div>
