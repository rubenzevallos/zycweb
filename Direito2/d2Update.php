...
if ($hs) {
  ...
  if ($_SESSION['cd_sessao']) {
    ...
    echo "Fatal Error - Sem sessão!<pre>";
    var_dump($_REQUEST);
  }
}
...
function update() {
  ...
  */
  ...
  if ($myRow = mysql_fetch_object($myQuery)) {
    ...
    (cd_noticia, hs_noticia, ds_url, tp_noticia_hash) VALUES
    ($cd_noticia,  MD5('$noticia->pagURL'), '$noticia->pagURL', 2)";
    ...
    $sql = "INSERT INTO tb_noticia_hash
    (cd_noticia, hs_noticia, ds_url, tp_noticia_hash) VALUES
    ($cd_noticia,  MD5('$noticia->pagArquivoPasta/$noticia->pagArquivoNome'), '$noticia->pagArquivoPasta/$noticia->pagArquivoNome', 1)";

    myQuery($sql);

    $cd_noticia_hash = mysql_insert_id();

    $sql = "UPDATE tb_noticia SET cd_noticia_hash = $cd_noticia_hash WHERE cd_noticia = $cd_noticia";
  ...
  if (count($comentarios)) {
    ...
    foreach ($comentarios as $key => $value) {
      ...
      if ($myRow = mysql_fetch_object($myQuery)) {
        ...
        ('$rec->comNome', '$rec->comEMail', MD5('$rec->comEMail'), SUBSTRING(MD5('$rec->comEMail'), 1, 10)";
  ...
  /*
	$sql = "UPDATE tb_noticia_hash SET hs_noticia = MD5(ds_url)";

	$myQuery = myQuery($sql);

	$sql = "UPDATE tb_noticia SET tb_noticia.cd_noticia_hash = (SELECT cd_noticia_hash FROM tb_noticia_hash where tb_noticia.cd_noticia = tb_noticia_hash.cd_noticia ORDER BY tb_noticia_hash.cd_noticia_hash DESC LIMIT 1)";
...
/*
UPDATE tb_noticia_hash
SET hs_noticia = MD5(ds_url)

UPDATE tb_noticia SET tb_noticia.cd_noticia_hash = (SELECT cd_noticia_hash FROM tb_noticia_hash where tb_noticia.cd_noticia = tb_noticia_hash.cd_noticia ORDER BY tb_noticia_hash.cd_noticia_hash DESC LIMIT 1)
